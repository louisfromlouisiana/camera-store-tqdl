...
import { useUpdateSpecificationMutation } from '../../../services/redux/query/products';
...
function UpdateSpecificationModal() {
  ...
  const [
    updateSpecification,
    {
      ...
      isSuccess: isSuccessUpdateSpecification,
      isLoading: isLoadingUpdateSpecification,
      isError: isErrorUpdateSpecification,
      error: errorUpdateSpecification,
    },
  ] = useUpdateSpecificationMutation();
  useEffect(() => {
    if (state.visibleUpdateSpecificationModal) {
      const { SKU, name, image, value, isSell, price, quantity } =
        state.visibleUpdateSpecificationModal;
      setForm({
        SKU: SKU,
        name: name,
        oldImage: image,
  ...
  const closeModal = useCallback(() => {
    setVisibleModal('visibleUpdateSpecificationModal');
    setForm({
      name: '',
      image: null,
      description: '',
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      await updateSpecification({
        id: state.visibleUpdateSpecificationModal?._id,
        body: data,
      });
    },
    [updateSpecification, form]
  );
  useEffect(() => {
    if (isSuccessUpdateSpecification && updateSpecificationData) {
      setForm({
        name: '',
        image: null,
        description: '',
      ...
      setVisibleModal({
        visibleToastModal: {
          type: 'success',
          message: updateSpecificationData?.message,
        },
      });
    }
    if (isErrorUpdateSpecification && errorUpdateSpecification) {
      setVisibleModal({
        visibleToastModal: {
          type: 'error',
          message: errorUpdateSpecification?.data?.message,
        },
      });
    }
  }, [
    isSuccessUpdateSpecification,
    updateSpecificationData,
    isErrorUpdateSpecification,
    errorUpdateSpecification,
    setVisibleModal,
  ]);
  return (
    <Modal>
          ...
          <div className='h-full flex flex-col gap-6 p-4 overflow-y-auto'>
            ...
            <div className='mt-auto flex justify-end items-stretch gap-4 font-bold'>
              <button
                ...
                onClick={closeModal}
                disabled={isLoadingUpdateSpecification}
              >
                Cancel
              </button>
              <button
