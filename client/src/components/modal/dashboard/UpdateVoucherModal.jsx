...
import { useUpdateVoucherMutation } from '../../../services/redux/query/products';
import {
  formatAndPreserveCursor,
  formatNumber,
} from '../../../services/utils/format';
function UpdateVoucherModal() {
  ...
  const [
    updateVoucher,
    {
      ...
      isError: isErrorUpdateVoucher,
      error: errorUpdateVoucher,
    },
  ] = useUpdateVoucherMutation();
  useEffect(() => {
    if (state.visibleUpdateVoucherModal) {
      const {
        SKU,
        description,
        discount,
        ...
        enabled,
        quantity,
      } = state.visibleUpdateVoucherModal;
      setForm(() => {
        return {
          SKU: SKU,
  ...
  useEffect(() => {
    if (isSuccessUpdateVoucher && updateVoucherData) {
      setVisibleModal({
        visibleToastModal: {
          type: 'success',
          message: updateVoucherData?.message,
        },
      });
    }
    if (isErrorUpdateVoucher && errorUpdateVoucher) {
      setVisibleModal({
        visibleToastModal: {
          type: 'error',
          message: errorUpdateVoucher?.data?.message,
        },
      });
    }
  }, [
    isSuccessUpdateVoucher,
    updateVoucherData,
    isErrorUpdateVoucher,
    errorUpdateVoucher,
    setVisibleModal,
  ]);
  return (
      ...
      >
        ...
        >
          <div className='px-4 py-4 sm:py-6 flex justify-between items-center gap-4 bg-neutral-200'>
            <h1 className='text-xl sm:text-2xl font-bold'>Update Voucher</h1>
            <button
              type='button'
              aria-label='close-modal'
              onClick={() => setVisibleModal('visibleUpdateVoucherModal')}
            >
