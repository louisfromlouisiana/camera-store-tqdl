...
import { useUpdateCategoryMutation } from '../../../services/redux/query/webInfo';
function UpdateCategoryModal() {
  ...
  useEffect(() => {
    if (state.visibleUpdateCategoryModal) {
      const { name, title, image, content } = state.visibleUpdateCategoryModal;
      setForm({
        name: name,
        title: title,
        image: null,
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      await updateCategory({
        id: state?.visibleUpdateCategoryModal?._id,
        body: data,
      });
    },
    [updateCategory, form]
  ...
  useEffect(() => {
    if (isSuccessUpdateCategory && updateCategoryData) {
      setVisibleModal({
        visibleToastModal: {
          type: 'success',
    ...
    if (isErrorUpdateCategory && errorUpdateCategory) {
      setVisibleModal({
        visibleToastModal: {
          type: 'error',
          message: errorUpdateCategory?.data?.message,
        },
      });
    }
  }, [
      ...
      >
        ...
        >
          <div className='px-4 py-4 sm:py-6 flex justify-between items-center gap-4 bg-neutral-200'>
            <h1 className='text-xl sm:text-2xl font-bold'>Update Category</h1>
            <button
              type='button'
              aria-label='close-modal'
              onClick={() => setVisibleModal('visibleUpdateCategoryModal')}
            >
