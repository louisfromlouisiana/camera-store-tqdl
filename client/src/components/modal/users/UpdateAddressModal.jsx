...
import { useUpdateAddressMutation } from '../../../services/redux/query/users';
import {
...
function UpdateAddressModal() {
  ...
  useEffect(() => {
    if (state.visibleUpdateAddressModal) {
      const { province, district, ward, address, phone, isDefault } =
  ...
  const handleSubmit = useCallback(
    async (e) => {
      e.preventDefault();
      await updateAddress({
        id: state.visibleUpdateAddressModal?._id,
        body: {
    ...
    [updateAddress, form, state.visibleUpdateAddressModal]
  );
  useEffect(() => {
    if (isSuccessUpdateAddress && updateAddressData) {
      setVisibleModal({
        visibleToastModal: {
          type: 'success',
          message: updateAddressData?.message,
        },
      });
    }
    if (isErrorUpdateAddress && errorUpdateAddress) {
      setVisibleModal({
        visibleToastModal: {
          type: 'error',
          message: errorUpdateAddress?.data?.message,
        },
      });
    }
  }, [
      ...
      >
        ...
        >
          <div className='px-4 py-4 sm:py-6 flex justify-between items-center gap-4 bg-neutral-200'>
            <h1 className='text-xl sm:text-2xl font-bold'>Update Address</h1>
            <button
              type='button'
                ...
                >
                  ...
                  <span
                    className={`absolute top-1/2 -translate-y-1/2 left-0 bg-white w-8 h-full rounded-full z-10 ${
                      form.isDefault ? 'translate-x-[140%]' : 'translate-x-0'
                    } transition-all duration-200`}
                  ></span>
                </button>
              </div>
